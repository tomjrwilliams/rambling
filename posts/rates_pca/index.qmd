...
For now, we'll go with the first of these, see [here](../rates_pca_smoothing) for a post on the second.
...
However, vanilla PCA - particularly in the eigen-decomposition form we used above - suffers from a number of limitations:

- incompatibility with missing data (hence the truncated history of our USD bond decomposition).

- factor loading volatility, given the relatively un-opionated nature of the decomposition (diagonal variance maximisation).

We'll deal with the first of these first via [matrix completion](../rates_pca_smoothing), and then more comprehensively by [combining](../rates_ksmoothing) this with a covariance [kernel](../rates_pca_k).

We'll deal with the second by moving from discrete loading vectors to [parametric factors](../rates_pf), on which we can impose a greater deal of prior knowledge.